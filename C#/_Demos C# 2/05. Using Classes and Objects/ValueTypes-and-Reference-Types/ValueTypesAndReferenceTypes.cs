...
class ValueTypesAndReferenceTypes
{
    static unsafe void Main()
    {
        ...
        Console.WriteLine("address(intNum) = 0x{0:x8} (in the stack)", (uint)&intNum);
        Console.Write("bytes(intNum)  = ");
        DumpBytes((byte*)&intNum, sizeof(int));
        Console.WriteLine();

        Console.WriteLine("address(dateTime) = 0x{0:x8} (in the stack)", (uint)&dateTime);
        ...
        fixed (int* pIntArrData = intArr)
        {
            ...
            Console.WriteLine("address(intArr) = 0x{0:x8} (int the heap)", (uint)pIntArr);
        ...
        fixed (char* pStrData = str)
        {
            ...
            Console.WriteLine("address(str) = 0x{0:x8} (in the heap)", (uint)pStr);
